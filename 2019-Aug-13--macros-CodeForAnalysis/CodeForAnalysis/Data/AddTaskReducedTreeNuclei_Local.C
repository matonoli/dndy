...
AliAnalysisTask *AddTaskReducedTreeNuclei_Local (TString prod="LHC16q")  {
  ...
  AliAnalysisTaskReducedTreeNuclei_Local *task = new AliAnalysisTaskReducedTreeNuclei_Local("TaskNuclei");
...
//  mgr->ConnectOutput(task, 3, cHeliumTree);
  mgr->ConnectOutput(task, 4, cTritonTree);
  //   mgr->ConnectOutput(task, 5, cHypertritonTree);
