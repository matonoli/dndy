...
enum EValue_t {
  kYield = 1,
  kYieldStat,
  kYieldSysHi, // only uncorrelated syst. obtained from random variation like stat. (renaming if the old code will be deleted)
  kYieldSysLo, // average of uncertainty obtained from shifting the spectrum by its correlated uncertainty up and down
  kMean,
  kMeanStat,
  kMeanSysHi, // only uncorrelated syst. obtained from random variation like stat. (renaming if the old code will be deleted)
  kMeanSysLo, // average of uncertainty obtained from shifting the spectrum by its correlated uncertainty up and down
  kExtra
};
...
TH1 * YieldMean_ReturnExtremeLowHisto(TH1 *hin, TH1 *hsift);
TH1 * YieldMean_ReturnExtremeHighHisto(TH1 *hin, TH1 *hsift);

TH1 *
YieldMean(TH1 *hstat, TH1 *hsys, TH1 *hsysUnCor, TF1 *f = NULL, Double_t min = 0., Double_t max = 10., Double_t loprecision = 0.01, Double_t hiprecision = 0.1, Option_t *opt = "0q",TString logfilename="log.root",Double_t minfit=0.0,Double_t maxfit=10.0)
{
  if(maxfit>max)
    max=maxfit;	
  if(minfit<min)
  ...
  ofstream myfile;
  myfile.open ("FitProtokoll.txt", ios::out | ios::app);
  
  myfile <<"Fit sys+stat for " <<f->GetName()<<endl;
  //  cout<<"NDF="<<f->GetNDF()<<" Chi^2="<<f->GetChisquare()<<" Chi^2/NDF="<<f->GetChisquare()/f->GetNDF()<<endl;
  myfile <<"NDF="<<f->GetNDF()<<" Chi^2="<<f->GetChisquare();
  if( f->GetNDF()!= 0) {
    myfile <<" Chi^2/NDF="<<f->GetChisquare()/f->GetNDF()<<endl;
  } else{
    myfile <<" Chi^2/NDF= inf" <<endl;
  }
  ...
  Double_t DefaultParams[10]= {}; // array size should be egual to the number of parameters ( f->GetNpa() ); for now too large size is chosen
  f->GetParameters(DefaultParams);
  ...
  // Calculated correlated part of the sys
  TH1* hsysCor = (TH1D*) hsys->Clone("hsysCor");
  for (int ibin=1; ibin <= hsysCor->GetNbinsX(); ibin++) {
    hsysCor->SetBinError(ibin,sqrt( pow(hsys->GetBinError(ibin),2) - pow(hsysUnCor->GetBinError(ibin),2)) );
  }
  
  TCanvas *cCanvasSys = new TCanvas("cCanvasYieldSys");
  cCanvasSys->Divide(2, 1);
  cCanvasSys->cd(1)->DrawFrame(min, 1.e-3, max, 1.e3);
  hsysCor->SetMarkerStyle(20);
  hsysCor->SetMarkerColor(1);
  hsysCor->SetMarkerSize(1);
  hsysCor->Draw("same");
  ...
  f->SetParameters(DefaultParams);
  
  TH1 *hhigh = YieldMean_ReturnExtremeHighHisto(htot, hsysCor);
  trials = 0;
  do {
  ...
  Double_t YieldSysHi = TMath::Abs(integral - hout->GetBinContent(kYield));
  Double_t MeanSysHi = TMath::Abs(mean - hout->GetBinContent(kMean));
  ...
  TH1 *hlow = YieldMean_ReturnExtremeLowHisto(htot, hsysCor);
  trials = 0;
  do {
  ...
  Double_t YieldSysLo = TMath::Abs(integral - hout->GetBinContent(kYield));
  Double_t MeanSysLo = TMath::Abs(mean - hout->GetBinContent(kMean));
  
  cout << "Yield: Shift up: " << YieldSysHi <<"; Shift down: " << YieldSysLo << endl;
  cout << "Mean pT: Shift up: " << MeanSysHi <<"; Shift down: " << MeanSysLo << endl;
  
  // calaculate average
  hout->SetBinContent(kYieldSysLo, (YieldSysHi+YieldSysLo)/2.);
  hout->SetBinContent(kMeanSysLo, (MeanSysHi+MeanSysLo)/2.);
  ...
  //  /*
  //   * systematic error hard (-> assumes anti-correlated uncertainty -> very conservative)
  //   */
  //
  ...
  //
  //  /*
  //   * systematic error soft (-> assumes anti-correlated uncertainty -> very conservative)
  //   */
  //
  ...
  // systematic uncertainties mean pT
  // uncorrelated part can be evaluated in a similar way as for the stat. uncertainty
  
  // Start with default parameter
  f->SetParameters(DefaultParams);

  TCanvas *cCanvasSystMeanPt = new TCanvas("cCanvasSystMeanPt");
  cCanvasSystMeanPt->Divide(2, 1);
  
  /* fit with uncorrelated systematic error */
  trials = 0;
  do {
    fitres = hsysUnCor->Fit(f, opt,"",minfit,maxfit);
    Printf("Trial: %d", trials++);
    if(trials > 10) {
      Printf("FIT DOES NOT CONVERGE IN LINE %d",__LINE__);
      break;
  ...
  while (fitres != 0);
  hlo = YieldMean_LowExtrapolationHisto(hsysUnCor, f, min, loprecision);
  hhi = YieldMean_HighExtrapolationHisto(hsysUnCor, f, max, hiprecision);
  ...
  for (Int_t irnd = 0; irnd < 100; irnd++) {
    /* get random histogram */
    TH1 *hrnd = YieldMean_ReturnRandom(hsysUnCor);
    /* extrapolation */
    TH1 *hrndlo = YieldMean_ReturnCoherentRandom(hlo);
    TH1 *hrndhi = YieldMean_ReturnCoherentRandom(hhi);
    /* integrate */
  ...
  cCanvasSystMeanPt->cd(1);
  hIntegral->Fit(gaus, "q");
  integral = hout->GetBinContent(kYield) * gaus->GetParameter(2) / gaus->GetParameter(1);
  hout->SetBinContent(kYieldSysHi, integral);
  
  cCanvasSystMeanPt->cd(2);
  hMean->Fit(gaus, "q");
  mean = hout->GetBinContent(kMean) * gaus->GetParameter(2) / gaus->GetParameter(1);
  hout->SetBinContent(kMeanSysHi, mean);
...
TH1 *
YieldMean_ReturnExtremeLowHisto(TH1 *hin, TH1 *hsift)
{
